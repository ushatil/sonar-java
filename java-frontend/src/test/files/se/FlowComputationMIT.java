...
class A {
  ...
  void exceptions2(Object o) {
    throwIfNull(o); // flow@ex2  {{Implies arg #1 is non-null}} flow@ex2 {{'throwIfNull' returns non-null}}
  ...
  void test(Object a) {
    if (trueIfNull(a)) { // flow@arg {{Implies arg #1 is null}}
  ...
  void exceptions(Object o) {
     try {
       throwIfNull(o); // flow@ex {{Implies arg #1 is null}} flow@ex {{'IllegalStateException' thrown by 'throwIfNull'}}
