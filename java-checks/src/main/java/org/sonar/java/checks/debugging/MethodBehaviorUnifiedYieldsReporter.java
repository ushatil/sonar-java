...
@Rule(key = "debugging-SE-UnifiedYields")
public class MethodBehaviorUnifiedYieldsReporter extends IssuableSubscriptionVisitor implements DebuggingVisitor {
  ...
  public void visitNode(Tree tree) {
    ...
    if (methodSymbol != null && methodSymbol.isMethodSymbol()) {
      ...
      reportMethodBehavior(reportTree, methodSymbol);
    }
  }
  ...
  private void reportMethodBehavior(Tree reportTree, @Nullable Symbol methodSymbol) {
    ...
    if (mb != null) {
      reportYields(reportTree, "Unified yields", mb.unifiedYields());
    }
  ...
  private static void prettyPrint(StringBuilder sb, String pathName, List<MethodYield> yields) {
    sb.append(pathName)
      .append(" (")
      .append(yields.size())
      .append("): [" + StringUtils.join(yields.stream().map(MethodBehaviorUnifiedYieldsReporter::prettyPrint).toArray(), ",") + "]");
  }
